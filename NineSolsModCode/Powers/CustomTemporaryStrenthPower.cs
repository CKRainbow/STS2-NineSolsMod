...
public abstract class CustomTemporaryStrenthPower : NineSolsModPower, ITemporaryPower
{
    ...
    public override LocString Title
    {
        get
        {
            ...
            if (originModel is not CardModel cardModel)
            {
                if (originModel is not PotionModel potionModel)
                {
    ...
    protected override IEnumerable<IHoverTip> AdditionalHoverTips
    {
        get
        {
            ...
            if (originModel is not CardModel card)
            {
                if (originModel is not PotionModel model)
                {
                    if (originModel is not RelicModel relic)
                    {
                        throw new InvalidOperationException();
                    }
    ...
    public override async Task AfterPowerAmountChanged(PlayerChoiceContext choiceContext, PowerModel power, decimal amount, Creature? applier, CardModel? cardSource)
    {
        if (!(amount == (decimal)Amount) && power == this)
        {
            ...
            {
                await PowerCmd.Apply<StrengthPower>(choiceContext, Owner, (decimal)Sign * amount, applier, cardSource, silent: true);
            }
        }
    }
